document.addEventListener("DOMContentLoaded",()=>{
  ...
  let quantity=Math.max(1,Math.min(20,parseInt(params.get("qty")||"1",10)||1));
  ...
  const getProductImage=product=>{
    ...
    if(media&&typeof media==="object"&&typeof media.image==="string"){
      return media.image;
    }

    if(media&&typeof media==="object"&&Array.isArray(media.images)&&media.images.length){
  ...
  const updateURL=()=>{
    ...
    url.searchParams.set("id",currentProduct.id);
  ...
  const renderSummary=()=>{
    ...
    const image=getProductImage(currentProduct);
    const size=getProductSize(currentProduct);

    $("summaryProductImage").src=image;
    $("summaryProductImage").alt=currentProduct.name||"Remify product";

    $("summaryProductName").textContent=
      currentProduct.name||"Remify Product";

    $("summaryProductPrice").textContent=displayPrice;

    $("summaryProductSize").textContent=size;

    $("summaryQuantity").textContent=quantity;

    $("summarySubtotal").textContent=displayTotal;

    $("summaryTotal").textContent=displayTotal;

    $("summaryItemCount").textContent=
      `${quantity} ${quantity===1?"item":"items"}`;

    document.title=
      `Checkout — ${currentProduct.name||"Remify"}`;
  ...
  const showError=message=>{
    ...
    $("checkoutPage").innerHTML=`
      <div class="checkout-error-state">
        <div>
          <span class="checkout-eyebrow">REMIFY</span>
          <h1>We couldn't load your order.</h1>
          <p>${escapeHTML(message)}</p>
          <a href="index.html">Back to Remify</a>
        </div>
      </div>
    `;
  ...
  const loadProduct=async()=>{
    ...
    try{
      ...
      console.error("Remify checkout error:",error);
  ...
  const changeQuantity=change=>{
    ...
    quantity=Math.max(1,Math.min(20,quantity+change));
...
form?.addEventListener("submit",async event=>{
  ...
  if(!valid){
    ...
    const firstInvalid=
      form.querySelector(".has-error input");
...
affiliateCode: localStorage.getItem("remifyAffiliateCode") || null
  ...
  `${API_BASE_URL}/api/payment/initialize`,
    ...
    if(
      ...
      throw new Error(
        "The secure payment service could not be loaded. Please refresh the page and try again."
      );
    }
  ...
  `${API_BASE_URL}/api/payment/verify`,
            ...
            console.error(
              "Remify payment verification error:",
              error
            );
            ...
            showCheckoutPaymentError(
              error.message||
              "We couldn't verify your payment. Please contact Remify if money was deducted."
            );
        ...
        onCancel:()=>{
          ...
          showCheckoutPaymentError(
            "The payment window was closed before the payment was completed. Your order has not been placed. You can try again."
          );
...
const checkoutStatusIcon=$("checkoutStatusIcon");
const checkoutStatusTitle=$("checkoutStatusTitle");
const checkoutStatusMessage=$("checkoutStatusMessage");
const checkoutStatusReference=$("checkoutStatusReference");
const checkoutStatusAction=$("checkoutStatusAction");
const checkoutStatusClose=$("checkoutStatusClose");
...
const showCheckoutStatus=({
  ...
  checkoutStatusIcon.textContent=icon;
...
const showCheckoutSuccess=reference=>{
...
const showCheckoutPaymentError=message=>{
...
checkoutStatusAction?.addEventListener(
  ...
  const menuToggle=$("checkoutMenuToggle");
  const menuClose=$("checkoutMenuClose");
...
if(cursorCanvas&&cursorSpark){
  ...
  const resizeCursorCanvas=()=>{
    ...
    cursorCanvas.width=cursorWidth*dpr;
    cursorCanvas.height=cursorHeight*dpr;
